...
const DraggableTextBox = ({
  ...
  const handleSize = Math.max(10, 14 / posterScale);
      ...
      {isSelected && (
        <div
          ...
          style={{
            ...
            background: accentColor, borderRadius: Math.max(2, 3 / posterScale),
            cursor: 'se-resize', zIndex: 10,
            border: `${Math.max(1, 1.5 / posterScale)}px solid white`,
          }}
        />
      )}
    </div>
    ...
    >
      <div className="relative bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden max-w-sm w-full">
        ...
        <div className="relative bg-slate-100 flex items-center justify-center" style={{ minHeight: 380 }}>
          ...
          <button
            onClick={prev}
            className="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/90 hover:bg-white rounded-full flex items-center justify-center shadow-md transition-all hover:scale-105"
          >
            <ChevronLeft className="w-5 h-5 text-slate-700" />
          </button>
          <button
            onClick={next}
            className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/90 hover:bg-white rounded-full flex items-center justify-center shadow-md transition-all hover:scale-105"
          >
        ...
        <div className="flex justify-center gap-1 py-2 px-4 flex-wrap">
          {templates.map((t, i) => (
            <button
              ...
              className={`rounded-full transition-all ${i === current ? 'w-4 h-1.5 bg-blue-600' : 'w-1.5 h-1.5 bg-slate-300 hover:bg-slate-400'}`}
            />
          ))}
        </div>
        ...
        <div className="flex gap-2 px-4 pb-4">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 border border-slate-200 hover:bg-slate-50 text-slate-600 rounded-xl text-sm font-semibold transition-all"
          >
            Đóng
          </button>
          <button
...
/* ─── Template Strip (horizontal, inside canvas) ─────────── */
const TemplateStrip = ({ templates, selectedTemplate, onSelect }) => {
  ...
  return (
    <>
      <div className="border-t border-slate-200/60 bg-white/70 backdrop-blur-sm">
        <div className="flex items-center gap-2 px-3 py-2">
          {/* Label */}
          <div className="flex items-center gap-1.5 shrink-0">
            <LayoutTemplate className="w-3.5 h-3.5 text-blue-600" />
            <span className="text-[10px] font-bold text-slate-600 uppercase tracking-wider hidden sm:block">Mẫu</span>
          ...
          <button
            onClick={scrollLeft}
            className="shrink-0 w-6 h-6 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-500 hover:text-slate-800 hover:border-slate-300 transition-all shadow-sm"
          >
                ...
                <div key={tpl.id} className="shrink-0 flex flex-col items-center gap-1">
                  <div
                    className={`relative rounded-lg overflow-hidden cursor-pointer transition-all duration-200 group ${
                      isActive ? 'ring-2 ring-blue-600 ring-offset-1' : 'hover:ring-2 hover:ring-slate-300 hover:ring-offset-1'
                    }`}
                  ...
                  >
                    ...
                    {/* Selected check */}
                    {isActive && (
                      <div className="absolute top-1 right-1 bg-blue-600 rounded-full p-px">
                    ...
                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <button
                        ...
                        className="bg-white text-slate-900 text-[9px] font-bold px-1.5 py-0.5 rounded-md shadow hover:bg-blue-600 hover:text-white transition-colors"
                      >
                        Xem
                      </button>
                    </div>
                  </div>
                  <span className="text-[9px] font-medium text-slate-500 truncate w-[52px] text-center leading-tight">
                    {tpl.name}
                  </span>
                </div>
              );
...
const PosterPreview = ({
  ...
  // Correct dimensions after scale (top-left origin)
  ...
  return (
    <div className="relative w-full h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-white/70 backdrop-blur-md border-b border-slate-200/60 z-10 gap-2 shrink-0">
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-white rounded-lg border border-slate-200/60 shadow-sm">
            ...
            <span className="text-[11px] font-bold text-slate-700 uppercase tracking-wide truncate max-w-[110px]">
              {selectedTemplate?.name || "Chọn mẫu"}
            </span>
          </div>
          {designMode && (
            <span className="hidden sm:flex items-center gap-1.5 px-2 py-1.5 text-[10px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-200/60 rounded-lg uppercase tracking-wider">
        ...
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex items-center gap-1 px-2 py-1.5 bg-white rounded-lg border border-slate-200/60 shadow-sm">
            <ZoomIn className="w-3 h-3 text-slate-400" />
            <span className="text-[10px] font-bold text-slate-500">{Math.round(previewScale * 100)}%</span>
          </div>
          <button
            onClick={handleDownload}
            disabled={isExporting}
            className="flex items-center gap-1.5 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition-all active:scale-95 disabled:opacity-50 shadow-md shadow-blue-600/20"
          >
      ...
      <div className="flex-1 flex items-center justify-center overflow-hidden relative">
        ...
        {/* Live badge */}
        <div className="absolute top-3 left-3 flex items-center gap-1.5 px-2.5 py-1 bg-white/85 backdrop-blur-sm rounded-full border border-slate-200/50 shadow-sm z-10">
          <div className="relative flex h-1.5 w-1.5">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
            <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-500" />
          </div>
          <span className="text-[9px] font-bold text-slate-600 uppercase tracking-widest">Live Canvas</span>
        </div>

        {designMode && !selectedBox && (
          <div className="absolute top-3 right-3 flex items-center gap-1 px-2 py-1 bg-blue-50 border border-blue-200/60 rounded-full z-10">
            <Move className="w-2.5 h-2.5 text-blue-500" />
            <span className="text-[9px] font-bold text-blue-600">Kéo để di chuyển</span>
          </div>
        )}

        {selectedTemplate ? (
          /* Wrapper takes the VISUAL (scaled) space */
          <div
            style={{
              width: scaledW,
              height: scaledH,
          ...
          >
            {/* Inner full-size poster scaled from top-left — no overflow */}
            <div
              style={{
                position: 'absolute',
                top: 0,
            ...
            >
              <div
                ref={el => {
                  ...
                  if (typeof posterRef === 'function') posterRef(el);
                  else if (posterRef) posterRef.current = el;
                }}
                data-canvas="true"
                style={{
              ...
              >
                ...
                <DraggableTextBox
                  label="Số điện thoại"
                  value={phone ? formatPhoneNumber(phone) : 'Số điện thoại / Zalo'}
                  boxStyle={phoneStyle}
          ...
          <div className="flex flex-col items-center gap-3 text-slate-400">
            <Eye className="w-14 h-14 stroke-1" />
            <p className="text-base font-bold">Chưa chọn mẫu thiết kế</p>
          </div>
        )}
      </div>

      {/* ── Template Strip (bottom of canvas) ── */}
      {templates && onSelectTemplate && (
        <TemplateStrip
          templates={templates}
          selectedTemplate={selectedTemplate}
